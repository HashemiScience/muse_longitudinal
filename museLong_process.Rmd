...
#files_raw = dir(psdpath, pattern="*raw_data.json")
print(paste('found ', length(files_psd), 'psd files'))

files_fooof_med_ch1 <- list.files(datapath, pattern="*fg_meditation_ch1.json", recursive = TRUE, full.names = TRUE)
files_fooof_cal_ch1 <- list.files(datapath, pattern="*fg_calibration_ch1.json", recursive = TRUE, full.names = TRUE)
files_fooof_med_ch4 <- list.files(datapath, pattern="*fg_meditation_ch4.json", recursive = TRUE, full.names = TRUE)
files_fooof_cal_ch4 <- list.files(datapath, pattern="*fg_calibration_ch4.json", recursive = TRUE, full.names = TRUE)

print(paste('found ', length(files_fooof_med_ch1), 'foof files'))
print(paste('found ', length(files_fooof_cal_ch1), 'foof files'))
...
with(subjdets, tapply(session_id, list(year_of_birth, gender), length))/25
...
PSD extracted from 120s to 300s (i.e. 2 - 5 min) of each meditation session (and all the time for calibration??)
in 5 second windows, with 50% overlap, averaging 30 windows to get one PSD. 
...
+ psd_std_calibration: st dev power at each frequency across session length
+ psd_max_calibration: max power estimate -- dont really need

same for meditation. 

```{r load psd files, eval = FALSE}
# moved this into readPSDfiles.R
...
start_time <- Sys.time()
for(ii in 1:length(files_psd)){ # loop per session file 
  # Give the input file name to the function.
  curfile = files_psd[ii]
  fname = strsplit(curfile, '/')
  session_id = substr(fname[[1]][length(fname[[1]])],1,nchar(fname[[1]][5])-10)
  ...
  for(ee in 1:length(raw_psd$psd_mean_meditation)){ # loop through number of epochs per subj
    psd_med_ch1 =tibble(session_id = session_id, 
                        ...
                        numPs = raw_psd$psd_num_meditation, 
...
# read in the FOOOF data. 
- Was fit using "fixed" mode
- 
```{r loading FOOOF data, echo = FALSE}
fooofdats = readFOOOFfiles(fooofpath)
