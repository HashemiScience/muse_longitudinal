...
def process_file(filename):
    ...
    AVG_NUM = 30 # psds to average prior to FOOOF extraction
    fmin, fmax = 2, 80  # look at PSD between 2 and 80 Hz
    PSD_TIMES = [120, 300] # times during meditation to extract PSD
    ...
    for id, stage in enumerate(['calibration', 'meditation']):
        ...
        psd_means = [np.mean(psds[n:n+30, :, :], keepdims=True, axis=0)
                     for n in np.arange(0, psds.shape[0]-30, 30)]
        psd_means = np.concatenate(psd_means, axis=0)
        psds_dict['psds_{}'.format(stage)] = psd_means
        psds_dict['freqs_{}'.format(stage)] = freqs

        #to be added: old alpha method
