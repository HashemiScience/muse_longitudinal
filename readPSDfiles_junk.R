readPSDfiles<-function(files2read){# Get a list of filenames (psd only)
...
for(ii in 1:length(files2read)){ # loop per session file   
  ...
  if(ii%%100==0){
  save(file = paste0('./psds_',previi, 'to', ii, '_',  start_time_String, '.Rdata'), list = ls(pattern = 'all_psd_*'))
